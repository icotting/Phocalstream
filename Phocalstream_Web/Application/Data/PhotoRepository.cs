        ...
        public System.Xml.XmlDocument CreateDeepZoomForSite(long siteID)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand("select Photos.ID,BlobID,Width,Height,ContainerID from Photos inner join CameraSites on Photos.Site_ID = CameraSites.ID where CameraSites.ID = @id", conn))
                {
                    ...
                    return CreateDeepZoomDocument(command);
                }
            }
        }
        ...
        private XmlDocument CreateDeepZoomDocument(SqlCommand command)
        {
            ...
            using (SqlDataReader reader = command.ExecuteReader())
            {
                ...
                while (reader.Read())
                {
                    ...
                    string imageSource = ( request == null ) ? string.Format("/dzc/{3}-dz/{4}.dzi", reader.GetString(4),reader.GetString(1))
                        : string.Format("{0}://{1}:{2}/dzc/{3}-dz/{4}.dzi", request.Url.Scheme,
                        request.Url.Host,
                        request.Url.Port,
                        reader.GetString(4),
                        reader.GetString(1));
